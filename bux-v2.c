                ...
                for (int j = 0; j < global_db.count; j++) {
                    if (strcmp(global_db.packages[j].name, name) == 0) {
                        ...
                        if (json_object_object_get_ex(jpkg, "version", &jversion)) {
                            strcpy(global_db.packages[j].installed_version, json_object_get_string(jversion));
                        }
                        ...
                        if (json_object_object_get_ex(jpkg, "store_hash", &jhash)) {
                            strcpy(global_db.packages[j].store_hash, json_object_get_string(jhash));
                        }
                        break;
                    }
                }
    ...
    if (json_object_object_get_ex(jroot, "repositories", &jrepos)) {
        ...
        for (int i = 0; i < len && global_db.repo_count < MAX_REPOS; i++) {
            ...
            if (json_object_object_get_ex(jrepo, "name", &jname) &&
                ...
                strcpy(repo->name, json_object_get_string(jname));
                strcpy(repo->url, json_object_get_string(jurl));
...
void parse_dependency(const char *dep_str, Dependency *dep) {
    ...
    if (strstr(space, ">=")) {
        sscanf(space, "%[^ ] >= %s", dep->name, version_str);
        strcpy(op, ">=");
        ...
        strcpy(dep->name, space);
        return;
    }

    strcpy(dep->operator, op);
    strcpy(dep->version, version_str);
}
...
int check_dependencies(Package *pkg, char error[MAX_LINE], int ignore_installed) {
    for (int i = 0; i < pkg->dependency_count; i++) {
        ...
        if (!dep_pkg) {
            sprintf(error, "Dependency '%s' not found", dep->name);
            return -1;
        }
        ...
        if (dep_pkg->is_installed && !ignore_installed) {
            if (dep->version[0] && !version_satisfies(dep_pkg->installed_version, dep->operator, dep->version)) {
                sprintf(error, "Dependency '%s' version mismatch: installed %s, required %s %s",
    ...
    for (int i = 0; i < pkg->conflict_count; i++) {
        ...
        if (conf_pkg && conf_pkg->is_installed) {
            if (!conf->version[0] || version_satisfies(conf_pkg->installed_version, conf->operator, conf->version)) {
                sprintf(error, "Conflicts with installed package '%s'", conf->name);
                return -1;
            }
        }
    }
...
int install_package_internal(Package *pkg, const char *version) {
    ...
    strcpy(pkg->store_hash, store_hash);
    strcpy(pkg->installed_version, target_version);
    pkg->is_installed = 1;
...
void check_updates() {
    ...
    for (int i = 0; i < global_db.count; i++) {
        ...
        if (cmp > 0) {
            ...
            strcpy(update->name, pkg->name);
            strcpy(update->old_version, pkg->installed_version);
            strcpy(update->new_version, latest->version);
...
void parse_packages_file(const char *filepath, const char *repo_name, int priority) {
    ...
    while (fgets(line, sizeof(line), fp)) {
        ...
        if (strncmp(line, "start_", 6) == 0) {
            ...
            strcpy(current->repo_name, repo_name);
            in_instructions = 0;
            in_versions = 0;
            in_dependencies = 0;
            in_conflicts = 0;
            ...
            strcpy(current->check_fetch, line + 6);
            current->has_check = 1;
            ...
            if (current->provides_count < MAX_PROVIDES) {
                sscanf(line, "%s %s", current->provides[current->provides_count].name,
...
void update_repository(const char *repo_name) {
    ...
    if (download_file_with_retry(url, temp_file, 3) == 0) {
        ...
        strftime(repo->last_update, sizeof(repo->last_update), "%Y-%m-%d %H:%M:%S", localtime(&now));
...
void clean_cache() {
    ...
    char cmd[MAX_CMD];
    snprintf(cmd, sizeof(cmd), "rm -rf %s/*", global_db.cache_dir);
    system(cmd);
...
void show_help() {
    ...
    printf("  showoff                       Install dev environment\n\n");
}
...
void run_dist(int argc, char **argv) {
    ...
    char cmd[MAX_CMD];
    snprintf(cmd, sizeof(cmd), "mount -t proc proc %s/proc 2>/dev/null", target);
    system(cmd);
    snprintf(cmd, sizeof(cmd), "mount -t sysfs sys %s/sys 2>/dev/null", target);
    system(cmd);
    snprintf(cmd, sizeof(cmd), "mount -o bind /dev %s/dev 2>/dev/null", target);
    system(cmd);
    snprintf(cmd, sizeof(cmd), "mount -o bind /dev/pts %s/dev/pts 2>/dev/null", target);
    system(cmd);
    snprintf(cmd, sizeof(cmd), "cp /etc/resolv.conf %s/etc/resolv.conf 2>/dev/null", target);
    system(cmd);

    char shell[64] = "/bin/bash";
    if (strcmp(distro_type, "alpine") == 0) strcpy(shell, "/bin/ash");

    snprintf(cmd, sizeof(cmd), "chroot %s %s", target, shell);
    system(cmd);

    snprintf(cmd, sizeof(cmd), "umount %s/dev/pts 2>/dev/null", target);
    system(cmd);
    snprintf(cmd, sizeof(cmd), "umount %s/dev 2>/dev/null", target);
    system(cmd);
    snprintf(cmd, sizeof(cmd), "umount %s/sys 2>/dev/null", target);
    system(cmd);
    snprintf(cmd, sizeof(cmd), "umount %s/proc 2>/dev/null", target);
    system(cmd);
...
void install_showoff() {
    printf("\033[1m\033[35mInstalling Showoff - The Ultimate Dev Environment\033[0m\n\n");
    
    printf("\033[34mStep 1: Installing system dependencies via apk...\033[0m\n");
    system("apk add git gcc musl-dev make zsh curl bash 2>/dev/null");
    
    printf("\n\033[34mStep 2: Installing Oh My Zsh...\033[0m\n");
    system("sh -c \"$(curl -fsSL https://raw.githubusercontent.com/ohmyzsh/ohmyzsh/master/tools/install.sh)\" \"\" --unattended");
    
    printf("\n\033[34mStep 3: Installing Starship prompt...\033[0m\n");
    system("curl -sS https://starship.rs/install.sh | sh -s -- -y");
    
    printf("\n\033[34mStep 4: Installing zsh-autosuggestions...\033[0m\n");
    char *home = getenv("HOME");
    char cmd[MAX_CMD];
    snprintf(cmd, sizeof(cmd), "git clone https://github.com/zsh-users/zsh-autosuggestions %s/.oh-my-zsh/custom/plugins/zsh-autosuggestions 2>/dev/null", home);
    system(cmd);
    
    printf("\n\033[34mStep 5: Installing zsh-syntax-highlighting...\033[0m\n");
    snprintf(cmd, sizeof(cmd), "git clone https://github.com/zsh-users/zsh-syntax-highlighting.git %s/.oh-my-zsh/custom/plugins/zsh-syntax-highlighting 2>/dev/null", home);
    system(cmd);
    
    printf("\n\033[34mStep 6: Configuring .zshrc...\033[0m\n");
    char zshrc[MAX_PATH];
    snprintf(zshrc, sizeof(zshrc), "%s/.zshrc", home);
    FILE *fp = fopen(zshrc, "w");
    if (fp) {
        fprintf(fp, "export ZSH=\"%s/.oh-my-zsh\"\n", home);
        fprintf(fp, "ZSH_THEME=\"\"\n");
        fprintf(fp, "plugins=(git zsh-autosuggestions zsh-syntax-highlighting)\n");
        fprintf(fp, "source $ZSH/oh-my-zsh.sh\n");
        fprintf(fp, "eval \"$(starship init zsh)\"\n");
        fprintf(fp, "export PATH=\"%s/.bux/profiles/default/bin:$PATH\"\n", home);
        fprintf(fp, "alias bux-update=\"bux update && bux upgrade-all\"\n");
        fprintf(fp, "alias bux-search=\"bux search\"\n");
        fprintf(fp, "alias bux-install=\"bux install\"\n");
        fclose(fp);
    }
    
    printf("\n\033[32m✓ Showoff installation complete!\033[0m\n");
    printf("\033[33mStart a new terminal or run: zsh\033[0m\n");
    printf("\033[33mYour .zshrc has been configured with:\033[0m\n");
    printf("  - Oh My Zsh (no theme)\n");
    printf("  - Starship prompt\n");
    printf("  - zsh-autosuggestions\n");
    printf("  - zsh-syntax-highlighting\n");
    printf("  - BUX profile PATH\n");
}
...
int main(int argc, char **argv) {
    ...
    if (strcmp(argv[1], "help") == 0 || strcmp(argv[1], "-h") == 0 || strcmp(argv[1], "--help") == 0) {
        ...
        else if (strcmp(argv[2], "showoff") == 0) install_showoff();
        ...
        run_audi(argc, argv);
    } else if (strcmp(argv[1], "showoff") == 0) {
        install_showoff();
    } else {
