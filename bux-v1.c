...
void load_installed_db() {
    ...
    if (json_object_object_get_ex(jroot, "packages", &jpackages)) {
        ...
        for (int i = 0; i < len; i++) {
            ...
            json_object *jname, *jversion, *jrepo, *jessential;
...
int install_package_internal(Package *pkg, const char *version) {
    ...
    char cache_dir[MAX_PATH];
    snprintf(cache_dir, sizeof(cache_dir), "%s/%s", global_db.cache_dir, pkg->name);
    ...
    snprintf(download_path, sizeof(download_path), "%s/download_%s", cache_dir, target_version);
    ...
    if (pkg->has_instructions) {
        ...
        execute_instructions(pkg->instructions, cache_dir);
    }
    ...
    snprintf(version_file, sizeof(version_file), "%s/version", cache_dir);
    ...
    printf("\033[32m✓ Successfully installed\033[0m %s %s\n", pkg->name, target_version);
    return 0;
}
...
void remove_package(const char *name) {
    ...
    char cache_dir[MAX_PATH];
    snprintf(cache_dir, sizeof(cache_dir), "%s/%s", global_db.cache_dir, name);
...
void show_help() {
    printf("\n\033[1mBUX Package Manager\033[0m\n\n");
...
int main(int argc, char **argv) {
    ...
    if (global_db.repo_count == 0) {
        add_repository("core", "https://bux-packages.example.com/core", 1000);
        add_repository("community", "https://bux-packages.example.com/community", 500);
    }
